...
export default function CreateApartment() {
    ...
    const [mapCenter, setMapCenter] = useState([51.5, 10])
    ...
    const geocodeAddress = async () => {
        ...
        setGeoLoading(true)
        setGeoError(null)
        const query = [house_number ? `${address} ${house_number}` : address, city, postal_code, COUNTRIES.find(([code]) => code === country)?.[1] ?? country].filter(Boolean).join(", ")
        try {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const textFields = ["title", "description", "country", "city", "address", "house_number", "postal_code", "property_type", "price_per_month", "currency", "min_rent_duration", "max_rent_duration", "max_residents", "bedrooms", "bathrooms", "size_of_property", "status", "latitude", "longitude"]
                ...
                <div style={{
                    ...
                    <div style={{marginTop: 8, marginBottom: 4}}>
                        <button
                            ...
                            disabled={geoLoading}
                            type="button"
                            style={{background: addressConfirmed ? "#17a589" : "var(--accent-grad)"}}
                        >
                    ...
                    {coords && (<div style={{
                            marginTop: 16, borderRadius: 10, overflow: "hidden", border: "2px solid var(--accent)"
                        }}>
                            <p style={{
                    ...
                    {previews.length > 0 && (<div style={{display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12}}>
